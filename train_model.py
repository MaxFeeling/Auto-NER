...
def train_one(config: Config, train_batches: List[Tuple], dev_insts: List[Instance], dev_batches: List[Tuple],
    ...
    cfig_path = os.path.join(config.bert_model_dir,
                             'bert_config.json')
    cfig = BertConfig.from_json_file(cfig_path) #return a instance of BertConfig
    cfig.device = config.device
    cfig.label2idx = config.label2idx
    cfig.label_size = config.label_size
    cfig.idx2labels = config.idx2labels
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Transformer CRF implementation") #输入实验参数
    ...
    # build label2idx and idx2label and output_id（label 列表）
    conf.build_label_idx(trains + devs)

    random.shuffle(trains)
    # set the prediction flag, if is_prediction is False, we will not update this label. O-->True
    for inst in trains:
...
if __name__ == "__main__":
    main()
#1.模型结构：multitask learning
#2.模型预训练问题：如何输出标签的概率分布
