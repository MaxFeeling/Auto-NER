...
def log_sum_exp_pytorch(vec: torch.Tensor) -> torch.Tensor:
    ...
    """
    maxScores, idx = torch.max(vec, 1)  #1是行
    maxScores[maxScores == -float("Inf")] = 0   #将maxscores中 = -inf 的数都赋值为0
...
def batching_list_instances(config: Config, insts: List[Instance]):
    ...
    return batched_data #word_seq_tensor,word_seq_len,label_seq_tensor,anotation_mask
...
def simple_batching(config, insts: List[Instance]) -> Tuple:
    ...
    word_seq_len = torch.LongTensor(list(map(lambda inst: len(inst.input.words), batch_data)))#[len1,len2,...]
